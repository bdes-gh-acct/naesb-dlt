...
export class VaultService {
  ...
  private async getVaultPath() {
    ...
    return await resolve(VAULT_SERVICE_NAME);
  }
  ...
  async getCert(path: string, serial: string) {
    return await this.get(`v1/${path}/cert/${serial}`);
  }
  ...
  async generateRole(
    ...
    return await this.post(`v1/${path}/roles/${name}`, payload);
  }
  async generateCert(
    path: string,
    role: string,
    ...
    return await this.post(`v1/${path}/issue/${role}`, payload);
  }
}
